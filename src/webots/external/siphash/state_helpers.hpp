...
namespace highwayhash {
  ...
  HH_INLINE void PaddedUpdate(const HH_U64 size, const char *remaining_bytes, const HH_U64 remaining_size, State *state) {
  ...
  // padded packet.
  template<class State> HH_INLINE void UpdateState(const char *bytes, const HH_U64 size, State *state) {
    // Feed entire packets.
  ...
  template<class String, class State> HH_INLINE void UpdateState(const String &s, State *state) {
  ...
  template<class State> HH_U64 ComputeHash(const typename State::Key &key, const char *bytes, const HH_U64 size) {
    State state(key);
    UpdateState(bytes, size, &state);
  ...
  template<class State> struct StringHasher {
    template<class String> HH_U64 operator()(const typename State::Key &key, const String &s) {
      State state(key);
      UpdateState(s, &state);
      return state.Finalize();
    }
