...
#include "arch_specific.hpp"
#include "compiler_specific.hpp"
#include "endianess.hpp"
#include "state_helpers.hpp"

namespace highwayhash {

  // Paper: https://www.131002.net/siphash/siphash.pdf
  template<int kUpdateIters, int kFinalizeIters> class SipHashStateT {
  public:
    using Key = HH_U64[2];
  ...
  private:
    ...
    template<HH_U64 bits> static HH_INLINE HH_U64 RotateLeft(const HH_U64 v) {
      const HH_U64 left = v << bits;
      const HH_U64 right = v >> (64 - bits);
      return left | right;
    }

    template<size_t rounds> HH_INLINE void Compress() {
