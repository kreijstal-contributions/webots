...
#define uart_send_static_text(msg)           \
  do {                                       \
    e_send_uart1_char(msg, sizeof(msg) - 1); \
    while (e_uart1_sending())                \
      ;                                      \
  } while (0)
#define uart_send_text(msg)              \
  do {                                   \
...
int main(void) {
  ...
  int i, j, n, speedr = 0, speedl = 0, positionr = 0, positionl = 0, LED_nbr, LED_action, accx = 0, accy = 0, accz = 0,
               selector = 0;
    ...
    if (c < 0) {  // binary mode (big endian)
      ...
      switch (buffer[0]) {
        ...
        case 'M':  // read floor sensors (optional)
          ...
          for (j = 0; j < 6; j++) {
            if (j % 2 == 0)
              buffer[j] = e_i2cp_read(0xC0, j + 1);
            else
              buffer[j] = e_i2cp_read(0xC0, j - 1);
          }
          e_i2cp_disable();
          sprintf(buffer, "m,%d,%d,%d\r\n", (unsigned int)(buffer[0] & 0xff) + ((unsigned int)buffer[1] << 8),
                  (unsigned int)(buffer[2] & 0xff) + ((unsigned int)buffer[3] << 8),
                  (unsigned int)(buffer[4] & 0xff) + ((unsigned int)buffer[5] << 8));
          uart_send_text(buffer);
#else
          ...
          break;
        case 'N':  // read proximity sensors
          sprintf(buffer, "n,%d,%d,%d,%d,%d,%d,%d,%d\r\n", e_get_prox(0) - ir_delta[0], e_get_prox(1) - ir_delta[1],
                  e_get_prox(2) - ir_delta[2], e_get_prox(3) - ir_delta[3], e_get_prox(4) - ir_delta[4],
                  e_get_prox(5) - ir_delta[5], e_get_prox(6) - ir_delta[6], e_get_prox(7) - ir_delta[7]);
          uart_send_text(buffer);
          break;
