...
unsigned int getBatteryValueRaw() {
  if (isEpuckVersion1_3()) {
    return e_acc_scan[2][e_last_acc_scan_id];  // the battery value on e-puck rev 1.3 is sampled on the same channel where the
                                               // accelerometer z axis was sampled on e-puck 1.0
  } else {
    return 0;
  }
}
