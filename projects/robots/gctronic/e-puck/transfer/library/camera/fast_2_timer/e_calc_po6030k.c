...
int e_po6030k_config_cam(unsigned int sensor_x1, unsigned int sensor_y1, unsigned int sensor_width, unsigned int sensor_height,
  ...
  if (e_po6030k_set_wx(sensor_x1 / zoom_sample, (ARRAY_ORIGINE_X + ARRAY_WIDTH + 1) / zoom_sample))
    return -1;

  if (e_po6030k_set_wy(sensor_y1 / zoom_sample, (ARRAY_ORIGINE_Y + ARRAY_HEIGHT + 1) / zoom_sample))
    return -1;

  if (e_po6030k_set_vsync(sensor_y1, ARRAY_ORIGINE_Y + ARRAY_HEIGHT))
    return -1;

  if (color_mode == GREY_SCALE_MODE) {
    switch (sampl_mode) {
      case PO_6030_MODE_VGA:
        e_po6030k_set_speed(PO_6030_SPEED_4);
        break;
      case PO_6030_MODE_QVGA:
        e_po6030k_set_speed(PO_6030_SPEED_2);
        break;
      case PO_6030_MODE_QQVGA:
        e_po6030k_set_speed(PO_6030_SPEED_1);
        break;
    }
  } else {
    switch (sampl_mode) {
      case PO_6030_MODE_VGA:
        e_po6030k_set_speed(PO_6030_SPEED_8);
        break;
      case PO_6030_MODE_QVGA:
        e_po6030k_set_speed(PO_6030_SPEED_4);
        break;
  ...
  if (e_po6030k_set_mode(color_mode, sampl_mode))
    return -1;

  /* set timer configuration */
  if (e_poxxxx_apply_timer_config(nb_lines, nb_pixels, e_po6030k_get_bytes_per_pixel(color_mode), pbp_w, pbp_h))
    return -1;
