...
void e_init_ad_scan(unsigned char only_micro) {
  ...
  if (selector == 10) {  // gumstix extension
    IPC2bits.ADIP =
      6;  // Give high priority to the ADC interrupt in order to be able to correctly read the sampled values of the
...
void __attribute__((__interrupt__, auto_psv)) _ADCInterrupt(void) {
  ...
  if (micro_only) {
    ...
    if (!((period_counter == ACC_PROX_PERIOD - 1) && (prox_number == 4)) &&
        !((period_counter == PULSE_PERIOD - 1) &&
          (prox_number ==
           4))) {  // the 2 gumstix ir sensors are sampled before the mic, so in that case the mic will be saved later.
    ...
    // prox channels copy (ambient and reflected)
      ...
      switch (prox_number) {
        ...
        // Additional prox 8 and 9 of the gumstix extension.
        case 4:
          e_ambient_and_reflected_ir[9] = *adc_ptr++;
