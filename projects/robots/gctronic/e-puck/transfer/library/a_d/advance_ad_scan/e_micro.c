 ...
 *							(use \ref MIC0_BUFFER, \ref MIC1_BUFFER , \ref MIC2_BUFFER  defined in
 *e_micro.h)
...
int e_get_micro_average(unsigned int micro_id, unsigned int filter_size) {
  ...
  if ((micro_id < 3) && (filter_size > 0) && (filter_size <= MIC_SAMP_NB)) {
    for (i = 0, j = (MIC_SAMP_NB - (filter_size - (e_last_mic_scan_id + 1))) % MIC_SAMP_NB; i < filter_size;
         i++, j = (j + 1) % MIC_SAMP_NB) {
 ...
 * Beware that from e-puck rev 1.3 the microphone sensitivity resulted a little bit different from the previous hardware
 *revision; some empirical tests show that the difference is about +/-15%.
 *
 * \param micro_id		micro's ID (0, 1, or 2)
 *							(use \ref MIC0_BUFFER, \ref MIC1_BUFFER , \ref MIC2_BUFFER  defined in
 *e_micro.h)
