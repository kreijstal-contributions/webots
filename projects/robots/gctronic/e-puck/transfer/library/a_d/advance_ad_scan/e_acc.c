...
int e_get_acc(unsigned int captor) {
  ...
  if (isEpuckVersion1_3()) {
    // Try to optimize the I2C requests: the time is based on the ADC ISR (64*1/16384=about 3.9 ms, that is 256 Hz as with
    // analog accelerometer), so we get and update for all 3 channels (with a single call) every 256 Hz independently of the
    // requested channel. This is to avoid especially situations in which the user requires all 3 channels (to do some maths)
    // one after the other, in this case it would requires 3 I2C calls that we can avoid this way.
    if (updateAccI2CCounter == 64) {
      getAllAxesAcc(&lastAccX, &lastAccY, &lastAccZ);
      updateAccI2CCounter = 0;
    }
    switch (captor) {
      // Shift by 4 the value received from the sensor (16 bits) to get 12 bits value as with the analog accelerometer (to
      // maintain compatibility with e-puck rev < 1.3) Moreover decrease the resulting value by 1/4 to get about the same
      // resolution for the g:
      // - e-puck rev < 1.3: 1 g = about 800
      // - e-puck rev 1.3: 1 g = about 1000 => decreased by 1/4 => about 750 (5% difference)
      case 0:
        tempValue = lastAccY - centre_y;  // exchange x and y axes in order to be consistent with e-puck rev < 1.3
        return (tempValue >> 4) - (tempValue >> 6);
      case 1:
        tempValue = lastAccX - centre_x;
        return (tempValue >> 4) - (tempValue >> 6);
      case 2:
        tempValue = lastAccZ - centre_z;
        return (tempValue >> 4) - (tempValue >> 6);
      default:
    ...
    switch (captor) {
      case 0:
        return (e_acc_scan[captor][e_last_acc_scan_id] - centre_x);
      case 1:
        return (e_acc_scan[captor][e_last_acc_scan_id] - centre_y);
      case 2:
        return (e_acc_scan[captor][e_last_acc_scan_id] - centre_z);
      default:
        return ((int)ANGLE_ERROR);
    }
  }
...
int e_get_acc_filtered(unsigned int captor, unsigned int filter_size) {
  ...
  if (isEpuckVersion1_3()) {
    ...
    if (isCalibratingFlag) {
      switch (captor) {
        case 0:
          return getYAxisAcc();  // exchange x and y axes in order to be consistent with e-puck 1.0;
        case 1:
          return getXAxisAcc();
        case 2:
          return getZAxisAcc();
      ...
      switch (captor) {
        ...
        // - e-puck rev < 1.3: 1 g = about 800
        // - e-puck rev 1.3: 1 g = about 1000 => decreased by 1/4 => about 750 (5% difference)
        // Add an offset of 2048 because it represents the value of 0 g with the analog accelerometer (to maintain compatibility
        // with e-puck rev < 1.3).
        case 0:
          tempValue = lastAccY;  // exchange x and y axes in order to be consistent with e-puck rev < 1.3;
    ...
    if ((captor < 3) && (filter_size > 0) && (filter_size <= ACC_SAMP_NB)) {
      for (i = 0, j = (ACC_SAMP_NB - (filter_size - (e_last_acc_scan_id + 1))) % ACC_SAMP_NB; i < filter_size;
           i++, j = (j + 1) % ACC_SAMP_NB) {
...
void calculate_acc_spherical(void) {
  ...
  inclination =
    90.0 - atan2f((float)(acc_z), sqrtf((float)(((long)acc_x * (long)acc_x) + ((long)acc_y * (long)acc_y)))) * CST_RADIAN;
