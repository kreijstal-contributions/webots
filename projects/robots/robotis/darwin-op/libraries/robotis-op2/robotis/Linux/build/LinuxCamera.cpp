...
LinuxCamera::LinuxCamera() : settings(CameraSettings()), camera_fd(-1), buffers(0), n_buffers(0) {
  DEBUG_PRINT = false;
...
int LinuxCamera::Initialize(int deviceIndex) {
  ...
  for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
    ...
    buffers[n_buffers].start = mmap(NULL, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, camera_fd, buf.m.offset);
...
void LinuxCamera::LoadINISettings(minIni *ini) {
  ...
  if ((value = ini->geti("Camera", "Brightness", -2)) != -2)
    newset.brightness = value;
  if ((value = ini->geti("Camera", "Contrast", -2)) != -2)
    newset.contrast = value;
  if ((value = ini->geti("Camera", "Saturation", -2)) != -2)
    newset.saturation = value;
  if ((value = ini->geti("Camera", "Gain", -2)) != -2)
    newset.gain = value;
  if ((value = ini->geti("Camera", "Exposure", -2)) != -2)
    newset.exposure = value;
