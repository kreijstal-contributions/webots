...
BulkReadData::BulkReadData() : start_address(0), length(0), error(-1) {
...
bool CM730::MX28InitAll() {
  ...
  if (WriteWord(JointData::ID_R_ELBOW, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_ELBOW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_ELBOW!\n");
  if (WriteWord(JointData::ID_R_ELBOW, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_ELBOW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_ELBOW!\n");

  if (WriteWord(JointData::ID_L_ELBOW, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_ELBOW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_ELBOW!\n");
  if (WriteWord(JointData::ID_L_ELBOW, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_ELBOW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_ELBOW!\n");

  if (WriteWord(JointData::ID_R_HIP_YAW, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_HIP_YAW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_HIP_YAW!\n");
  if (WriteWord(JointData::ID_R_HIP_YAW, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_HIP_YAW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_HIP_YAW!\n");

  if (WriteWord(JointData::ID_L_HIP_YAW, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_HIP_YAW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_HIP_YAW!\n");
  if (WriteWord(JointData::ID_L_HIP_YAW, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_HIP_YAW), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_HIP_YAW!\n");

  if (WriteWord(JointData::ID_R_HIP_ROLL, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_HIP_ROLL), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_HIP_ROLL!\n");
  if (WriteWord(JointData::ID_R_HIP_ROLL, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_HIP_ROLL), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_HIP_ROLL!\n");

  if (WriteWord(JointData::ID_L_HIP_ROLL, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_HIP_ROLL), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_HIP_ROLL!\n");
  if (WriteWord(JointData::ID_L_HIP_ROLL, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_HIP_ROLL), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_HIP_ROLL!\n");

  if (WriteWord(JointData::ID_R_HIP_PITCH, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_HIP_PITCH), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_HIP_PITCH!\n");
  if (WriteWord(JointData::ID_R_HIP_PITCH, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_HIP_PITCH),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_HIP_PITCH!\n");

  if (WriteWord(JointData::ID_L_HIP_PITCH, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_HIP_PITCH), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_HIP_PITCH!\n");
  if (WriteWord(JointData::ID_L_HIP_PITCH, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_HIP_PITCH),
                0) != CM730::SUCCESS)
  ...
  if (WriteWord(JointData::ID_R_KNEE, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_KNEE), 0) !=
      CM730::SUCCESS)
  ...
  if (WriteWord(JointData::ID_L_KNEE, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_KNEE), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_KNEE!\n");

  if (WriteWord(JointData::ID_R_ANKLE_PITCH, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_ANKLE_PITCH),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_ANKLE_PITCH!\n");
  if (WriteWord(JointData::ID_R_ANKLE_PITCH, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_ANKLE_PITCH),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_ANKLE_PITCH!\n");

  if (WriteWord(JointData::ID_L_ANKLE_PITCH, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_ANKLE_PITCH),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_ANKLE_PITCH!\n");
  if (WriteWord(JointData::ID_L_ANKLE_PITCH, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_ANKLE_PITCH),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_ANKLE_PITCH!\n");

  if (WriteWord(JointData::ID_R_ANKLE_ROLL, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_R_ANKLE_ROLL),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of R_ANKLE_ROLL!\n");
  if (WriteWord(JointData::ID_R_ANKLE_ROLL, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_R_ANKLE_ROLL),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of R_ANKLE_ROLL!\n");

  if (WriteWord(JointData::ID_L_ANKLE_ROLL, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_L_ANKLE_ROLL),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of L_ANKLE_ROLL!\n");
  if (WriteWord(JointData::ID_L_ANKLE_ROLL, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_L_ANKLE_ROLL),
                0) != CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of L_ANKLE_ROLL!\n");

  if (WriteWord(JointData::ID_HEAD_PAN, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_HEAD_PAN), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of HEAD_PAN!\n");
  if (WriteWord(JointData::ID_HEAD_PAN, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_HEAD_PAN), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CCW limit of HEAD_PAN!\n");

  if (WriteWord(JointData::ID_HEAD_TILT, MX28::P_CW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CW_LIMIT_HEAD_TILT), 0) !=
      CM730::SUCCESS)
    fprintf(stderr, " Fail to change CW limit of HEAD_TILT!\n");
  if (WriteWord(JointData::ID_HEAD_TILT, MX28::P_CCW_ANGLE_LIMIT_L, MX28::Angle2Value(Kinematics::CCW_LIMIT_HEAD_TILT), 0) !=
      CM730::SUCCESS)
