...
input: an image img
output: no output
effects: modify m_result. m_result is an image of the same size that img. _result->m_ImageData[i] is then 1 if the pixel n. i is
correct and  else.
*/
void ColorFinder::Filtering(Image *img) {
  ...
  for (int i = 0; i < img->m_NumberOfPixels; i++) {
    ...
    if (((int)s >= m_min_saturation) && ((int)s <= m_max_saturation) && ((int)v >= m_min_value) && ((int)v <= m_max_value)) {
      if (h_min <= h_max) {
...
void ColorFinder::LoadINISettings(minIni *ini, const std::string &section) {
  int value = -2;
  if ((value = ini->geti(section, "hue", INVALID_VALUE)) != INVALID_VALUE)
    m_hue = value;
  if ((value = ini->geti(section, "hue_tolerance", INVALID_VALUE)) != INVALID_VALUE)
    m_hue_tolerance = value;
  if ((value = ini->geti(section, "min_saturation", INVALID_VALUE)) != INVALID_VALUE)
    m_min_saturation = value;
  if ((value = ini->geti(section, "max_saturation", INVALID_VALUE)) != INVALID_VALUE)
    m_max_saturation = value;
  if ((value = ini->geti(section, "min_value", INVALID_VALUE)) != INVALID_VALUE)
    m_min_value = value;
  if ((value = ini->geti(section, "max_value", INVALID_VALUE)) != INVALID_VALUE)
    m_max_value = value;

  double dvalue = -2.0;
  if ((dvalue = ini->getd(section, "min_percent", INVALID_VALUE)) != INVALID_VALUE)
    m_min_percent = dvalue;
  if ((dvalue = ini->getd(section, "max_percent", INVALID_VALUE)) != INVALID_VALUE)
    m_max_percent = dvalue;
