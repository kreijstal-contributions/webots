...
void ImgProcess::RGBtoHSV(FrameBuffer *buf) {
  ...
  for (int i = 0; i < buf->m_RGBFrame->m_Width * buf->m_RGBFrame->m_Height; i++) {
    ...
    if ((tv != 0) && (diffvmin != 0)) {
      ...
      if (tv == ir)
        th = (ig - ib) * 60 / diffvmin;
      else if (tv == ig)
        th = 120 + (ib - ir) * 60 / diffvmin;
      else
        th = 240 + (ir - ig) * 60 / diffvmin;
      if (th < 0)
        th += 360;
      th &= 0x0000FFFF;
    } else {
...
void ImgProcess::Erosion(Image *img) {
  ...
  for (y = 1; y < (img->m_Height - 1); y++) {
    for (x = 1; x < (img->m_Width - 1); x++) {
      temp_img[y * img->m_Width + x] =
        img->m_ImageData[(y - 1) * img->m_Width + (x - 1)] & img->m_ImageData[(y - 1) * img->m_Width + (x)] &
        img->m_ImageData[(y - 1) * img->m_Width + (x + 1)] & img->m_ImageData[(y)*img->m_Width + (x - 1)] &
        img->m_ImageData[(y)*img->m_Width + (x)] & img->m_ImageData[(y)*img->m_Width + (x + 1)] &
        img->m_ImageData[(y + 1) * img->m_Width + (x - 1)] & img->m_ImageData[(y + 1) * img->m_Width + (x)] &
...
void ImgProcess::Erosion(Image *src, Image *dest) {
  ...
  for (y = 1; y < (src->m_Height - 1); y++) {
    for (x = 1; x < (src->m_Width - 1); x++) {
      dest->m_ImageData[y * src->m_Width + x] =
        src->m_ImageData[(y - 1) * src->m_Width + (x - 1)] & src->m_ImageData[(y - 1) * src->m_Width + (x)] &
        src->m_ImageData[(y - 1) * src->m_Width + (x + 1)] & src->m_ImageData[(y)*src->m_Width + (x - 1)] &
        src->m_ImageData[(y)*src->m_Width + (x)] & src->m_ImageData[(y)*src->m_Width + (x + 1)] &
        src->m_ImageData[(y + 1) * src->m_Width + (x - 1)] & src->m_ImageData[(y + 1) * src->m_Width + (x)] &
...
void ImgProcess::HFlipYUV(Image *img) {
  ...
  for (int h = 0; h < img->m_Height; h++) {        /*line iterator*/
    for (int w = sizeline - 1; w > 0; w = w - 4) { /* pixel iterator */
      line[w - 1] = *pframe++;
      line[w - 2] = *pframe++;
      line[w - 3] = *pframe++;
      line[w] = *pframe++;
