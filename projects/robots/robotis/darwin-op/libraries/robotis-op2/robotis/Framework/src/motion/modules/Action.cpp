...
#ifdef WEBOTS
  // Olivier.Michel@cyberbotics.com added the following line to allow opening a readonly file located in the Webots installation
  // directory.
  // This is mainly problematic on Windows
  if (action == 0)
    action = fopen(filename, "rb");
#endif
        ...
        if (m_Joint.GetEnable(bID) == true) {
          if (ipMovingAngle1024[bID] == 0)
            ...
            if (bSection == PRE_SECTION) {
              ...
              ipAccelAngle1024[bID] =
                (short)((((long)(ipLastOutSpeed1024[bID] + (iSpeedN >> 1)) * wUnitTimeCount * 144) / 15) >> 9);

              m_Joint.SetValue(bID, wpStartAngle1024[bID] + ipAccelAngle1024[bID]);
            } else if (bSection == MAIN_SECTION) {
              m_Joint.SetValue(
                bID, wpStartAngle1024[bID] + (short int)(((long)(ipMainAngle1024[bID]) * wUnitTimeCount) / wUnitTimeNum));
        ...
        if (m_Joint.GetEnable(bID) == true) {
          if (bpFinishType[bID] == NONE_ZERO_FINISH) {
            if ((wUnitTimeTotalNum - wAccelStep) == 0)  // 등속 구간이 전혀 없다면
              ...
              ipMainAngle1024[bID] = (short)((((long)(ipMovingAngle1024[bID] - ipAccelAngle1024[bID])) * wUnitTimeNum) /
    ...
    if (bSection == PRE_SECTION) {
      ...
      for (bID = JointData::ID_R_SHOULDER_PITCH; bID < JointData::NUMBER_OF_JOINTS; bID++) {
        if (m_Joint.GetEnable(bID) == true) {
          ...
          if (bpFinishType[bID] == ZERO_FINISH)
            ipMainSpeed1024[bID] =
              (short int)((lMovingAngle_Speed1024Scale_256T_2T - lStartSpeed1024_PreTime_256T) / lDivider2);
          else
            ipMainSpeed1024[bID] =
              (short int)((lMovingAngle_Speed1024Scale_256T_2T - lStartSpeed1024_PreTime_256T) / lDivider1);
